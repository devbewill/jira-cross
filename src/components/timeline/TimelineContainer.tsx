        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-3 pl-4 border-l border-linear-border">
            {[
              ...
              { className: "bg-linear-accent",  label: "In Progress" },
              { className: "bg-linear-todo",    label: "Todo"        },
            ].map(({ className, label }) => (
              <span key={label} className="flex items-center gap-1.5 text-[11px] font-medium text-linear-textSecondary">
                <span className={`w-2.5 h-2.5 rounded-full flex-shrink-0 ${className}`} />
                {label}
              </span>
            ))}
          </div>
          ...
          {!todayVisible && (
            <button
              onClick={goToToday}
              className="px-3 py-1.5 text-[11px] font-semibold rounded-lg transition-all duration-150 bg-linear-accent text-white shadow-accent-glow hover:bg-linear-accentHover"
            >
              → Today
            </button>
          )}
        ...
        <div className="w-56 flex-shrink-0 flex flex-col bg-linear-surface border-r border-linear-border">
          ...
          >
            {boards.map((board, i) => (
              ...
              >
                <span className="text-[12px] font-semibold break-words mb-0.5 text-linear-text">
                  {board.name || board.key}
                </span>
                <span className="text-[11px] font-medium tabular-nums mb-2 text-linear-textDim">
                  {board.epics.length}{" "}
                ...
                <ul className="flex flex-col gap-1">
                  {board.epics.map((epic) => (
                    ...
                    >
                      <span className="text-[9px] font-semibold flex-shrink-0 mt-[1px] transition-colors text-linear-textDim">
                        {epic.key}
                      </span>
                      <span className="text-[10px] font-medium leading-tight truncate transition-colors text-linear-textMuted">
                        {epic.summary}
                      </span>
                    </li>
                  ))}
