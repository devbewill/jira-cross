  ...
  return (
    ...
    >
      <span
        ...
        style={{ backgroundColor: statusDotColor(story.statusCategory), border: DOT_BORDER }}
      />
...
export function ReleaseIssuesPanel({ release, onClose }: ReleaseIssuesPanelProps) {
  ...
  const stats = useMemo(() => {
    ...
    const inProgress = stories.filter((s) => s.statusCategory === "in-progress" || s.statusCategory === "indeterminate" as string).length;
        ...
        >
          <div className="flex items-start justify-between gap-3 mb-3">
            <div className="flex-1 min-w-0">
              ...
              {release.description && (
                <p className="text-[11px] text-[#717171] mt-1 leading-snug">{release.description}</p>
              )}
            </div>
          ...
          {!loading && stats.total > 0 && (
            <div>
              <div className="flex w-full h-[6px] rounded-full overflow-hidden mb-2" style={{ backgroundColor: "#E5E7EB" }}>
                {stats.done > 0 && (
                  <div style={{ width: `${(stats.done / stats.total) * 100}%`, backgroundColor: DOT_DONE }} />
                )}
                {stats.inProgress > 0 && (
                  <div style={{ width: `${(stats.inProgress / stats.total) * 100}%`, backgroundColor: DOT_IN_PROGRESS }} />
                )}
              </div>
              <div className="flex gap-3 text-[10px] font-semibold flex-wrap">
                {stats.done > 0 && (
                  <span className="flex items-center gap-1">
                    <span className="w-[8px] h-[8px] rounded-full flex-shrink-0" style={{ backgroundColor: DOT_DONE }} />
                    <span className="text-[#1A1A1B]">{stats.done} done</span>
                  </span>
                )}
                {stats.inProgress > 0 && (
                  <span className="flex items-center gap-1">
                    <span className="w-[8px] h-[8px] rounded-full flex-shrink-0" style={{ backgroundColor: DOT_IN_PROGRESS }} />
                    <span className="text-[#1A1A1B]">{stats.inProgress} in progress</span>
                  </span>
                )}
                {stats.todo > 0 && (
                  <span className="flex items-center gap-1">
                    <span className="w-[8px] h-[8px] rounded-full flex-shrink-0" style={{ backgroundColor: DOT_TODO }} />
