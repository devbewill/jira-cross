...
import { daysLabel, formatDateCompact, formatDateShort } from "@/lib/utils/format-utils";
    ...
    <>
      <div
        ...
        style={{
          ...
          outline: isSelected ? `2px solid ${cfg.borderHex}` : "none",
          outlineOffset: "2px",
        }}
      ...
      >
        ...
        {/* Colored bar */}
        <div
          className="relative w-full rounded-lg overflow-hidden transition-all duration-150 shadow-linear-sm"
          style={{
            height:          `${BAR_H}px`,
        ...
        >
          <div className="relative z-10 h-full flex items-center justify-between px-2.5">
            <span
              ...
              style={{ backgroundColor: "rgba(0,0,0,0.08)", color: cfg.textHex }}
            >
              {release.projectKey}
            </span>
            ...
            <div className="flex items-center gap-2">
              ...
              {release.releaseDate && (
                <span className="text-[9px] font-medium opacity-70" style={{ color: cfg.textHex }}>
      ...
      {tooltipPos && mounted && createPortal(
        ...
        >
          <div className="bg-linear-surface rounded-xl px-3 py-2.5 flex flex-col gap-1.5 border border-linear-border shadow-popover" style={{ minWidth: "200px" }}>
            ...
            {release.description && (
              <span className="text-[10px] text-linear-textSecondary leading-snug">{release.description}</span>
            )}

            <div className="grid grid-cols-2 gap-x-4 gap-y-0.5 mt-1 pt-1.5 border-t border-linear-border">
              <div>
                <span className="block text-[8px] font-semibold uppercase tracking-widest text-linear-textDim">Start</span>
                <span className="text-[10px] font-semibold text-linear-text">{release.startDate ? formatDateShort(release.startDate) : "\u2014"}</span>
              </div>
              <div>
                <span className="block text-[8px] font-semibold uppercase tracking-widest text-linear-textDim">Release</span>
                <span className="text-[10px] font-semibold text-linear-text">{formatDateShort(release.releaseDate)}</span>
              </div>
            </div>
